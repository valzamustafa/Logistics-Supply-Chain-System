...
export function WarehouseCard({ 
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-100/90 overflow-hidden hover:border-cyan-500/50 transition">
      {/* Header */}
      ...
      <div className="p-5">
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className={`rounded-xl p-3 text-center ${hasLowStock ? 'bg-yellow-500/20' : 'bg-slate-50/50'}`}>
            <AlertCircle className={`w-5 h-5 mx-auto mb-1 ${hasLowStock ? 'text-yellow-400' : 'text-slate-500'}`} />
            <p className={`text-xl font-bold ${hasLowStock ? 'text-yellow-400' : 'text-slate-900'}`}>{stats.lowStock}</p>
            <p className={`text-xs ${hasLowStock ? 'text-yellow-400/70' : 'text-slate-500'}`}>Low Stock</p>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full px-3 py-2 bg-slate-200 hover:bg-slate-100 rounded-lg text-slate-900 text-sm transition flex items-center justify-center gap-1"
        >
