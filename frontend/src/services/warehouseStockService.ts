...
export const warehouseStockService = {
  ...
  assignProductToWarehouse: async (warehouseId: number, data: AssignProductToWarehouseDto) => {
    const response = await api.post<WarehouseStock>(
      `/api/warehousestock/warehouse/${warehouseId}/assign`,
      data
    );
    return response;
  },

  getUnassignedProducts: (warehouseId: number) => 
    api.get<Product[]>(`/api/warehousestock/warehouse/${warehouseId}/unassigned-products`),
