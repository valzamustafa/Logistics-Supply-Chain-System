...
import { Plus, Edit, Trash2, XCircle, CheckCircle, Building2, Box, Users, MapPin, Phone, Eye, X, Save, AlertCircle, TrendingDown, TrendingUp, Search, Package, Key, Truck, RefreshCw, Navigation, Clock, User as UserIcon } from 'lucide-react';
...
const [showVehicleModal, setShowVehicleModal] = useState(false);
 ...
 useEffect(() => {
  ...
  const handleShipmentUpdate = (updatedShipment: Shipment) => {
    setShipments((prev) => {
      const existingIndex = prev.findIndex((shipment) => shipment.id === updatedShipment.id);
      if (existingIndex >= 0) {
  ...
  useEffect(() => {
    ...
    const initSignalR = async () => {
      try {
        ...
        console.error('Manager dashboard SignalR connection failed:', err);
      }
    };
  ...
  useEffect(() => {
    ...
    const activeShipments = shipments.filter((s) => s.status?.toLowerCase().includes('in transit') || s.status?.toLowerCase().includes('processing')).length;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      showToast('error', error.message || 'Failed to load data. Please check if all services are running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateWarehouse = async () => {
    ...
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Warehouse', title: 'Warehouse Created', message: `Warehouse ${warehouseForm.name} created`, actionUrl: '/manager' }).catch(() => {});
    } catch (error: any) {
      console.error('Failed to create warehouse:', error);
      showToast('error', error.message || 'Failed to create warehouse');
    }
  };
  ...
  const handleUpdateWarehouse = async () => {
    ...
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Warehouse', title: 'Warehouse Updated', message: `Warehouse ${warehouseForm.name} updated`, actionUrl: '/manager' }).catch(() => {});
    } catch (error: any) {
      console.error('Failed to update warehouse:', error);
      showToast('error', error.message || 'Failed to update warehouse');
    }
  };
  ...
  const handleDeleteWarehouse = async (id: number) => {
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Warehouse', title: 'Warehouse Deleted', message: `Warehouse deleted`, actionUrl: '/manager' }).catch(() => {});
    } catch (error: any) {
      console.error('Failed to delete warehouse:', error);
      showToast('error', error?.response?.data?.message || 'Cannot delete warehouse with existing stock. Transfer or remove stock first.');
    }
  };
  ...
  const handleToggleStatus = async (id: number, isActive: boolean) => {
    try {
      ...
      await refreshWarehouses();
      showToast('success', `Warehouse ${!isActive ? 'activated' : 'deactivated'} successfully!`);
    } catch (error) {
      console.error('Failed to toggle status:', error);
      showToast('error', 'Failed to update warehouse status');
    }
  };
  ...
  const handleCreateZone = async () => {
    ...
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Zone', title: 'Zone Created', message: `Zone ${zoneForm.zoneName} created`, actionUrl: '/manager' }).catch(() => {});
    } catch (error) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Zone', title: 'Zone Deleted', message: `Zone deleted`, actionUrl: '/manager' }).catch(() => {});
        } catch (error) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Staff', title: 'Staff Removed', message: `Staff removed from warehouse`, actionUrl: '/manager' }).catch(() => {});
        } catch (error) {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'UserManagement', title: 'User Created', message: `User ${userForm.email} created`, actionUrl: '/manager' }).catch(() => {});
    } catch (error: any) {
  ...
  const handleEditUser = async () => {
    ...
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'UserManagement', title: 'User Updated', message: `User updated`, actionUrl: '/manager' }).catch(() => {});
    } catch (error: any) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'UserManagement', title: 'User Deleted', message: `User deleted`, actionUrl: '/manager' }).catch(() => {});
        } catch (error: any) {
  ...
  const statItems = [
    { label: 'Total Orders', value: stats.totalOrders.toString(), icon: <Package className="w-7 h-7 text-white" />, color: 'from-cyan-400 to-blue-500' },
    { label: 'Pending Orders', value: stats.pendingOrders.toString(), icon: <Clock className="w-7 h-7 text-white" />, color: 'from-yellow-400 to-orange-500' },
    { label: 'Revenue', value: `$${(stats.totalRevenue / 1000).toFixed(1)}K`, icon: <TrendingUp className="w-7 h-7 text-white" />, color: 'from-green-400 to-emerald-500' },
    { label: 'Low Stock', value: stats.lowStockItems.toString(), icon: <AlertCircle className="w-7 h-7 text-white" />, color: 'from-red-400 to-orange-500' },
    { label: 'Shipments', value: stats.totalShipments.toString(), icon: <Truck className="w-7 h-7 text-white" />, color: 'from-purple-400 to-pink-500' },
    { label: 'Active Shipments', value: stats.activeShipments.toString(), icon: <Box className="w-7 h-7 text-white" />, color: 'from-blue-400 to-cyan-500' },
  ];
    ...
    <div className="flex flex-col gap-8 p-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <ExportButtons data={{ orders, shipments, inventory, warehouses, products, users }} />
          <button
            onClick={() => {
              setEditingWarehouse(null);
              setWarehouseForm({ name: '', location: '', phone: '' });
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md border border-slate-200">
            ...
            <div className="p-4 border-t border-slate-200 flex gap-3">
              ...
              <button onClick={handleAssignWarehouse} className="flex-1 btn-primary">Assign to Warehouse</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md border border-slate-200">
            ...
            <div className="p-4 border-t border-slate-200 flex gap-3">
              ...
              <button onClick={handleUpdatePermissions} className="flex-1 btn-primary">Save Permissions</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md border border-slate-200">
            ...
            <div className="p-4 border-t border-slate-200 flex gap-3">
              ...
              <button onClick={editingWarehouse ? handleUpdateWarehouse : handleCreateWarehouse} className="flex-1 btn-primary">{editingWarehouse ? 'Update' : 'Create'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md border border-slate-200">
            ...
            <div className="p-4 border-t border-slate-200 flex gap-3">
              ...
              <button onClick={handleCreateZone} className="flex-1 btn-primary">Create Zone</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md border border-slate-200">
            ...
            <div className="p-4 border-t border-slate-200 flex gap-3">
              ...
              <button onClick={handleAssignStaff} className="flex-1 btn-primary">Assign Staff</button>
            </div>
          </div>
        </div>
      )}
