...
const [vehicleStats, setVehicleStats] = useState<Record<number, { status: string; progress: number; location: string }>>({});
  ...
  const mapShipmentStatusToPurchaseOrderStatus = (shipmentStatus: string): string => {
    if (!shipmentStatus) return 'Pending';
  ...
  const handleShipmentUpdate = (updatedShipment: Shipment & any) => {
    ...
    const targetOrderId = updatedShipment.purchaseOrderId ?? updatedShipment.PurchaseOrderId ?? updatedShipment.orderId ?? updatedShipment.OrderId;
    if (targetOrderId != null) {
      const mappedStatus = mapShipmentStatusToPurchaseOrderStatus(updatedShipment.status || updatedShipment.Status || 'Pending');
      setDashboard((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
          orders: prev.orders.map((order) =>
            order.id === targetOrderId ? { ...order, status: mappedStatus } : order
          ),
        };
      });
    }
  ...
  const resolveOrderUpdateTargetId = (update: OrderUpdateEvent | any): number | undefined => {
    ...
    return (
      parseId(update.purchaseOrderId) ??
      parseId(update.PurchaseOrderId) ??
      parseId(update.orderId) ??
      parseId(update.OrderId)
    );
  };

  const refreshSupplierDashboard = async () => {
    try {
      const dashboardData = await supplierService.getDashboard();
      setDashboard(dashboardData);
    } catch (err) {
      console.error('Failed to refresh supplier dashboard after order update:', err);
    }
  };

  const handleOrderUpdate = (update: OrderUpdateEvent | any) => {
    const targetOrderId = resolveOrderUpdateTargetId(update);
    const newStatus = update.purchaseOrderStatus || update.PurchaseOrderStatus || update.status || update.Status;

    console.debug('Supplier dashboard order update event:', update, { targetOrderId, newStatus });
    ...
    setDashboard((prev) => {
      ...
      return {
        ...prev,
        orders: prev.orders.map((order) =>
          order.id === targetOrderId ? { ...order, status: newStatus } : order
        ),
      };
    });
