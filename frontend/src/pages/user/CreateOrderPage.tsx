...
export function CreateOrderPage() {
  ...
  const validateOrderInputs = () => {
    ...
    if (!shippingAddress.trim()) {
      showToast('error', 'Please enter a shipping address');
      return false;
    }

    if (selectedWarehouse === null) {
      showToast('error', 'Please select a warehouse for this order');
      return false;
    }
    ...
    if (invalidItem) {
      ...
      showToast('error', `${invalidItem.product.name} exceeds available quantity in the selected warehouse. Available: ${availableQuantity}, in cart: ${invalidItem.quantity}.`);
      return false;
    }
  ...
  const handleInvoiceSubmit = async () => {
    if ((paymentMethod === 'BankTransfer' && !bankAccount.trim()) || !billingName.trim() || !billingEmail.trim()) {
      showToast('error', 'Please fill all invoice fields');
      return;
    }
