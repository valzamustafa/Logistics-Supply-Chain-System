      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-100/90 p-6 backdrop-blur">
        ...
        <div className="space-y-3">
          {orders.slice(0, 5).map((order) => (
            <div key={order.id} className="flex justify-between items-center p-3 rounded-lg bg-slate-100/80 hover:bg-slate-100/80 transition">
              <div>
                <p className="text-slate-900 font-medium">{order.orderNumber}</p>
                <p className="text-xs text-slate-500">
                  {new Date(order.orderDate).toLocaleDateString()} • {order.items?.length || 0} items
                </p>
              </div>
              <div className="text-right flex items-center gap-3">
                <p className="text-slate-900 font-semibold">${order.totalAmount.toFixed(2)}</p>
        ...
        {shipments.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {shipments.slice(0, 2).map((shipment) => (
              <div key={shipment.id} className="rounded-2xl border border-slate-200 bg-slate-50/30 p-4">
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div>
                    <p className="text-sm text-slate-500">Shipment</p>
                    <p className="text-slate-900 font-semibold">{shipment.trackingNumber}</p>
                  </div>
