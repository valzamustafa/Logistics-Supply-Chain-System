...
export function WarehouseStaffDashboard() {
  ...
  const handleUpdateStock = async () => {
    ...
    try {
      ...
      if (user?.id) await notificationService.sendNotification({ userId: user.id, type: 'Stock', title: 'Stock Updated', message: `Stock updated for product ${selectedItem.productId} in warehouse ${selectedItem.warehouseId}`, actionUrl: '/warehouse' }).catch(() => {});
    } catch (error) {
