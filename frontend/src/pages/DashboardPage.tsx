    ...
    <div className="flex flex-col gap-8 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="rounded-2xl border border-slate-200 bg-slate-100/90 p-6 backdrop-blur transition hover:border-slate-600"
          >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <div>
            ...
            {filteredShipments.length === 0 ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-100/90 p-8 text-center">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-500">{shipment.shippingAddress || 'No address specified'}</p>
        ...
        <div className="space-y-4">
          ...
          {selectedShipment ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-100/90 p-6 backdrop-blur space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Tracking Number</p>
                  <p className="text-lg font-semibold text-slate-900">{selectedShipment.trackingNumber}</p>
                </div>
                <span
              ...
              <div className="h-32 rounded-lg bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-slate-200 flex items-center justify-center text-5xl">
                📦
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-slate-200">
                <div>
                  ...
                  <p className="text-sm text-slate-900 font-medium">{selectedShipment.orderId}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase">Destination</p>
                  <p className="text-sm text-slate-900 font-medium">{selectedShipment.shippingAddress || 'No address specified'}</p>
                </div>
                <div>
                ...
                {selectedShipment.driverName && (
                  <div>
                    ...
                    <p className="text-sm text-slate-900 font-medium">{selectedShipment.driverName}</p>
                  </div>
                )}
              </div>

              <button className="w-full rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-4 py-3 font-semibold text-slate-900 transition hover:from-cyan-400 hover:to-blue-400">
                View Details
              </button>
            </div>
          ) : (
            <div className="rounded-2xl border border-slate-200 bg-slate-100/90 p-6 text-center">
              <p className="text-slate-500">Select a shipment to view details</p>
            </div>
          )}
        </div>
      </div>
